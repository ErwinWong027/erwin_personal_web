...
### v1.9 — 履历时间轴样式升级 (Round 19)

**User Input**: 履历分页的教育经历和工作经历做成参考图样式（彩色渐变卡片 + 时间轴布局）。
...
**AI Output (2 个新文件，1 个修改文件)**:

**新建文件**:
| 文件路径 | 用途 |
|---------|------|
| `src/components/sections/TimelineCard.tsx` | 可配置的彩色时间轴卡片组件 |
| `src/app/globals.css` | 添加渐变背景色变量定义 |

**修改文件**:
| 文件路径 | 修改内容 |
|---------|---------|
| `src/components/sections/Experience.tsx` | 完全重构为时间轴布局，支持左右交替、彩色渐变、悬停动画 |

**技术实现**:

**TimelineCard 组件特性**:
- **Props 设计**: `title`, `role`, `period`, `description`, `highlights`, `color`, `position`, `index`
- **布局逻辑**: 通过 `position` 属性控制左右交替（`left` / `right`）
- **时间轴节点**: 菱形装饰（`rotate-45`），根据位置动态定位
- **动画效果**: 
  - 滚动进入动画（`AnimateOnScroll` + `delay={index * 0.1}`）
  - 悬停效果（`hover:shadow-xl hover:-translate-y-1`）

**Experience 组件重构**:
- **数据结构**: 从翻译键映射改为直接硬编码中文内容
- **教育经历**（3 条）:
  1. 法国 EDHEC 商学院 - 粉色渐变 (`from-pink-200 to-pink-300`)
  2. 江西财经大学 - 紫色渐变 (`from-purple-200 to-purple-300`)
  3. 伦敦大学 - 蓝色渐变 (`from-blue-200 to-blue-300`)
- **工作经历**（4 条）:
  1. 求职方舟 - 黄色渐变 (`from-yellow-200 to-yellow-300`)
  2. 字节跳动 - 绿色渐变 (`from-green-200 to-green-300`)
  3. Iterative Games - 橙色渐变 (`from-orange-200 to-orange-300`)
  4. CrossPath - 青色渐变 (`from-cyan-200 to-cyan-300`)

**样式细节**:
- **时间轴线**: 渐变背景 (`from-slate-200 via-slate-300 to-slate-200`)，仅桌面端显示
- **卡片样式**: `rounded-xl border-2 border-slate-800 shadow-lg`
- **要点列表**: 圆点标记 (`rounded-full bg-slate-800`)，每项前缀小圆点
- **响应式**: 移动端单列，桌面端左右交替

**Tailwind CSS v4 配置**:
在 `globals.css` 中添加自定义颜色变量：
```css
@theme {
...
**User Input**: 
1. 标题放左上角，不能和时间轴线重合
2. Section 的 padding 太大，标题可以往上
3. 教育经历和工作经历模块要分区

**AI Output**:
- **标题对齐**: `text-center` → `text-left`，左上角对齐
- **间距优化**: 
  - `section` 的 `py-24` → `py-16`（减少顶部 padding）
  - 标题 `mb-8` → `mb-6`（更靠近卡片）
- **模块分隔**: 添加渐变分隔线（`h-px bg-gradient-to-r from-transparent via-slate-300 to-transparent`）
- **间距调整**: 教育经历模块 `mb-16` → `mb-20`，分隔线 `mb-20`

**修改文件**:
| 文件路径 | 修改内容 |
|---------|---------|
| `src/components/sections/Experience.tsx` | 标题左对齐、减少 padding、添加分隔线 |

**核心价值**:
履历模块从"网格卡片"升级为"时间轴故事线"，通过彩色渐变、左右交替布局、时间轴线串联，视觉化呈现个人成长轨迹，更符合招聘方的阅读习惯。
